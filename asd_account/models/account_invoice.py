...
class AccountInvoice(models.Model):
    ...
    @api.multi
    def _recompute_invoice_tax(self, invoice_id, skip_invoice_ids=[]):
        if not skip_invoice_ids or (skip_invoice_ids and invoice_id.id not in skip_invoice_ids):
            for rec in invoice_id:
                ...
                for line in rec.payment_move_line_ids:
                    if line.journal_id.id in (50, 51):
    ...
    @api.one
    def action_correction_invoice_tax(self):
        self._recompute_invoice_tax(invoice_id=self)

    @api.multi
    def cron_correction_invoice_tax(self, start_date='2022-01-01', end_date='2022-03-31', limit=1, skip_invoice_ids=[]):
        ...
        if skip_invoice_ids:
            # repr() untuk mengeliminasi ekstra koma pada tuple yang berisi 1 data
            skip_ai_ids_temp = "(" + ", ".join(repr(val) for val in tuple(skip_invoice_ids)) + ")"
            query += " AND ai.id not in "  + skip_ai_ids_temp
        query += """
            GROUP BY
                ai.id
            ORDER BY
        ...
        self._recompute_invoice_tax(invoice_id=invoice_id, skip_invoice_ids=skip_invoice_ids)
    ...
    def faktur_pajak_create(self):
        if not self.nomor_faktur_id and self.type == 'out_invoice':
            ...
            result = self._cr.dictfetchall()
            faktur_id = self.env['nomor.faktur.pajak'].search([('id','=',result[0]['id'])]) if result else False
