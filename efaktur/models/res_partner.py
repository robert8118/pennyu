...
class res_partner(models.Model):
    ...
    def _get_npwp_info(self):
        ...
        if partner_address and partner_address.street:
            ...
            self.npwp_name = partner_address.name.upper()
            self.npwp_info = partner_address
    ...
    npwp_info = fields.Many2one('res.partner', string='NPWP Info', compute='_get_npwp_info', store=False)
    npwp_name = fields.Char(string='NPWP Name', compute='_get_npwp_info', store=False)
...
#                     self.country_id = kelurahan.kabupaten_id.state_id.country_id.id
    ...
    def _get_name_npwp(self):
        ...
        partner_address = self
        is_child = False
        if self.is_npwp or self.is_npwp_pribadi:
            partner_address = self
            partner = self.name
        elif self.child_ids:
            addr_npwp = ''
        ...
        if self.is_npwp and partner_address and partner_address.name:
            ...
            if self.is_npwp_pribadi and self.ktp and partner_address.nama_npwp_pribadi and (self.npwp == '00.000.000.0-000.000' or self.npwp == '0'):
                partner = self.ktp+'#NIK#NAMA#'+partner_address.nama_npwp_pribadi
            elif self.ktp and partner_address.name and (self.npwp == '00.000.000.0-000.000' or self.npwp == '0'):
                partner = self.ktp+'#NIK#NAMA#'+partner_address.name
            elif self.is_npwp_pribadi and partner_address.nama_npwp_pribadi and (self.npwp != '00.000.000.0-000.000' or self.npwp != '0'):
                partner = partner_address.nama_npwp_pribadi 
