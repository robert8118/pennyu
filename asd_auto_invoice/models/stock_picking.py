...
class StockPicking(models.Model):
    ...
    def auto_invoice(self, picking_id=None, return_status=False):
        """Automate create invoice when validate Picking"""
        ...
        if sp_id.group_id:
            ...
            return_type = self._get_return_type(
                pick_type_code=sp_id.picking_type_id.code, is_return=return_status
            )
            ...
            if return_status:
                ...
                ai_domain = [("origin", "=", f"{sp_origin.group_id.name}: {sp_origin_name}")]
                ai = invoice_obj.search(ai_domain, limit=1)
                if ai.state == "draft":
                    sp_id.decrease_qty_invoice(ai, order_type)
                    return False

            invoice_id = ai if return_status else False
            invoice_data = self._prepare_data_account_invoice(
            ...
            for move in sp_id.move_lines:
                if move.quantity_done:
                    invoice_data_line = self._prepare_data_account_invoice_line(
                        order_type=order_type,
                        move=move,
                        invoice_id=inv,
                        order_id=order_id,
    ...
    def _prepare_data_account_invoice(
        self, order_type, return_type, picking_id, invoice_id, data_dict
    ):
        ...
        return_of_do_return = (
            order_type == "purchase" and not order_id and sale_group_id
        )
        ...
        origin = (
            f"{order_id.name}: {picking_id.name}"
            if not return_type
            else invoice_id.number
        )
        ...
        journal_domain = [("company_id", "=", company_id), ("type", "=", order_type)]
        ...
        if order_type == "purchase":
            if sale_group_id and order_id != sale_group_id:
                data_dict.update({"reference": order_id.partner_ref})
        elif order_type == "sale":
            data_dict.update(
                {
                    "partner_id": order_id.partner_invoice_id.id
                    or order_id.partner_id.id,
    ...
    def _prepare_data_account_invoice_line(
        self, order_type, move, invoice_id, order_id
    ):
        account_obj = self.env["account.account"]
        data_line = {
        ...
        if order_type == "purchase":
            ...
            data_line.update(
                {"sale_line_id": order_line.id, "discount": order_line.discount}
            )

        product_name = f"{order_id.name}: {move.product_id.display_name}"
        account_domain = [
            ("code", "=", account_code),
