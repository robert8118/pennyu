...
class FpInvoiceExport(models.TransientModel):
    ...
    def _get_header_fapr(self, headers):
        ...
        rows = [fapr_ids[line] for line in headers]
        return rows
    ...
    def _get_header_lt(self, headers):
        ...
        rows = [lt_ids[line] for line in headers]
        return rows
    ...
    def _get_invoice_line(self, headers):
        ...
        invoice_type = context.get("default_type", False)
        for inv in invoices:
            # Raise error based on several conditions:
            # Invoice state
            if inv.state in ("draft", "cancel"):
                invoice_state = dict(self._fields["state"].selection).get(self.state)
                raise UserError(_(f"Cannot export {invoice_state} Invoice! {inv.origin}"))
            # Invoice amount_tax
            if not inv.amount_tax:
                raise UserError(_("Make sure select invoice which has taxes!"))
            # Invoice default_type
            if invoice_type != inv.type:
                if invoice_type in ("out_invoice", "out_refund"):
                    ...
                    type = "Masukan"
                    itype = "Keluaran"
                raise UserError(_(f"Cannot export FP {itype} Invoice {inv.number} as it on FP {type}!"))
            ...
            # base dictionaries data
            fapr_ids = {}
            if invoice_type in ("out_invoice","out_refund", "in_invoice","in_refund"):
                inv_ids = {
            ...
            if invoice_type in ("out_invoice","out_refund"):
                ...
                for line in inv.invoice_line_ids:
                    if line.invoice_line_tax_ids:
                        ...
                        row = [line_ids[line] for line in headers]
                        rows.append(list(row))
        return rows
